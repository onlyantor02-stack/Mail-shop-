...
Option 1:
Paste tokens, 1 token per line

Option 2:
Upload .xlsx/.xls file
1 token per row/cell`
    );
  }
...
async function handleStockXlsx(msg) {
  ...
  if (!st || st.type !== "stockTokens") {
    return bot.sendMessage(chatId, "First select product from Add Stock.");
  }

  const fileName = msg.document.file_name || "";

  if (!fileName.endsWith(".xlsx") && !fileName.endsWith(".xls")) {
    return bot.sendMessage(chatId, "Only .xlsx or .xls file allowed.");
  }
  ...
  try {
    ...
    if (!tokens.length) {
      return bot.sendMessage(chatId, "No token found in XLSX file.");
    }
...
${db.products[st.product].stock.length}`,
    ...
    console.log("XLSX upload error:", error.message);
    return bot.sendMessage(chatId, "XLSX upload failed. Try again.");
  }
}
