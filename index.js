...
function adminMenu() {
  return {
    reply_markup: {
      keyboard: [
        ["➕ Add Mail", "🎉 Set Mail Price"],
        ["📥 Add Stock", "📊 Live Stock"],
        ["💰 Edit Balance", "💳 Deposit Requests"],
        ["📜 Deposit History", "📲 Change Payment Method"],
        ["👤 User Info", "👤 User Message"],
        ["✏️ Send Broadcast", "⛔ Ban / Unban"],
        ["📦 Orders", "🔙 User Menu"]
      ],
      resize_keyboard: true
    }
...
// মেইন মেসেজ হ্যান্ডলার (একত্রিত করা হয়েছে)
bot.on("message", async (msg) => {
  ...
  // ১. ফাইল এবং স্ক্রিনশট হ্যান্ডলিং
  ...
  // ২. যদি কোনো অ্যাক্টিভ স্টেট থাকে, তবে স্টেট হ্যান্ডলার রান হবে
  if (state[chatId] && text !== "") {
    return handleState(msg);
  }

  // ৩. অ্যাডমিন প্যানেল কমান্ডস
  if (isAdmin(chatId)) {
    if (text === "➕ Add Mail") {
      state[chatId] = { type: "addMail" };
      return bot.sendMessage(chatId, "Send:\n\nProduct Name | Price\n\nExample:\nNetflix Token | 120");
    }
    if (text === "🎉 Set Mail Price") {
    ...
    if (text === "🟣 Change bKash") {
      ...
      return bot.sendMessage(chatId, "Send new bKash number:");
    }
    if (text === "🟠 Change Nagad") {
      state[chatId] = { type: "changeNagad" };
      return bot.sendMessage(chatId, "Send new Nagad number:");
    }
    if (text === "🔵 Change Rocket") {
      state[chatId] = { type: "changeRocket" };
      return bot.sendMessage(chatId, "Send new Rocket number:");
    }
    if (text === "👤 User Info") {
      state[chatId] = { type: "userInfo" };
      return bot.sendMessage(chatId, "Send user ID:");
    }
    ...
    if (text === "✏️ Send Broadcast") {
      ...
      return bot.sendMessage(chatId, "Send broadcast message:");
    }
    if (text === "⛔ Ban / Unban") {
...
async function handleState(msg) {
  ...
  if (st.type === "addMail") {
    ...
    saveDB();
    state[chatId] = null;
    return bot.sendMessage(chatId, "✅ Mail Added", adminMenu());
  }
  ...
  if (st.type === "buyQuantity") {
    ...
    if (product.stock.length < quantity) {
      return bot.sendMessage(chatId, "Not enough stock.");
    }
  ...
  if (st.type === "userMessageId") {
    ...
    return bot.sendMessage(chatId, "Send message:");
  }
  if (st.type === "userMessageText") {
    await bot.sendMessage(st.uid, text);
    state[chatId] = null;
    return bot.sendMessage(chatId, "✅ Message sent.");
  }
  ...
  if (st.type === "banUser") {
    ...
    saveDB();
    state[chatId] = null;
    return bot.sendMessage(chatId, db.users[uid].banned ? "✅ User banned" : "✅ User unbanned");
  }
}
...
async function handleDepositScreenshot(msg) {
  ...
  return bot.sendMessage(chatId, "✅ Screenshot received.");
}
...
bot.onText(/\/approve (.+)/, (msg, match) => {
  ...
  // ক্র্যাশ প্রোটেকশন চেক (যদি ইউজার কোনো কারণে ডেটাবেজে না থাকে)
