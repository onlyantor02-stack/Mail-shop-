...
// বাধ্যতামূলক চ্যানেল জয়েন চেক করার ফাংশন
async function isJoined(chatId) {
  if (isAdmin(chatId)) return true; // অ্যাডমিনের চেক করার প্রয়োজন নেই
  try {
    const member = await bot.getChatMember(CHANNEL_ID, chatId);
    const status = member.status;
    return status === "creator" || status === "administrator" || status === "member";
  } catch (err) {
    // যদি বট চ্যানেলে অ্যাডমিন না থাকে তবে এরর এড়াতে ট্রু রিটার্ন করবে
    console.log("Join Check Error (Make sure bot is admin in channel):", err.message);
    return false;
  }
}

// জয়েন না থাকলে এই মেসেজ ও ইনলাইন বাটন দেখাবে
...
bot.on("message", async (msg) => {
  ...
  // অ্যাডমিন ছাড়া সাধারণ ইউজারদের যেকোনো মেসেজে বাধ্যতামূলক জয়েন চেক করা হবে
  if (!isAdmin(chatId)) {
    const joined = await isJoined(chatId);
    if (!joined) {
      state[chatId] = null; // স্টেট ক্লিয়ার করা হলো
      return sendJoinMessage(chatId);
    }
  }

  // স্ক্রিনশট রিসিভার স্টেট চেক
...
bot.on("callback_query", async (q) => {
  ...
  // অন্য সব অ্যাকশনের আগে জয়েন স্ট্যাটাস ভেরিফাই করা হবে (অ্যাডমিন বাদে)
  if (!isAdmin(chatId)) {
    const joined = await isJoined(chatId);
    if (!joined) {
...
async function handleState(msg) {
  ...
  if (st.type === "userMessageText") {
    try {
      ...
      bot.sendMessage(chatId, "❌ Failed to send message (User blocked bot?).", adminMenu());
    }
    state[chatId] = null;
    return;
  }
...
async function handleDepositScreenshot(msg) {
  ...
  return bot.sendMessage(chatId, "✅ Txn ID & Screenshot received. Processing your deposit.", userMenu());
}
...
async function handleStockXlsx(msg) {
  ...
  if (!st || !db.products[st.product]) {
    state[chatId] = null;
    return bot.sendMessage(chatId, "Session error or missing selected productContext.");
  }
  ...
  try {
    ...
    console.log(err);
    return bot.sendMessage(chatId, "❌ Failed to parse Excel file structure.", adminMenu());
  }
}

// ডেটাবেজ ব্যাকআপ ফাইল পাঠানোর ফাংশন
...
// ডেটাবেজ রিস্টোর/ইনপুট করার হ্যান্ডলার
async function handleRestoreDB(msg) {
  ...
  try {
    // শুধুমাত্র .json ফাইল এক্সেপ্ট করবে
    if (!msg.document.file_name.endsWith(".json")) {
      return bot.sendMessage(chatId, "❌ Invalid file! Please upload a valid JSON (`database.json`) backup file.", cancelKeyboard());
    }
    ...
    // বেসিক স্ট্রাকচার চেক ভ্যালিডেশন
    if (!parsedData.users || !parsedData.products || !parsedData.deposits || !parsedData.orders) {
      try { fs.unlinkSync(realPath); } catch {}
      return bot.sendMessage(chatId, "❌ Failed to restore! The file is missing important database fields.", cancelKeyboard());
    }

    // ব্যাকআপ ফাইল রাইট করা
    fs.writeFileSync(DB_FILE, JSON.stringify(parsedData, null, 2));
    db = parsedData; // লাইভ ভেরিয়েবল আপডেট
    ...
    console.log(err);
    return bot.sendMessage(chatId, "❌ Failed to parse or apply the backup file.", adminMenu());
  }
}
